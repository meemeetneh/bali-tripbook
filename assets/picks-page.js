(() => {
  ...
  const render=filter=>{
    ...
    list.innerHTML=places.map(place=>`<article class="pick-card"><a href="place.html?id=${encodeURIComponent(place.id)}"><img src="${esc(place.image)}" alt="${esc(place.name)}" loading="lazy"></a><div><a href="place.html?id=${encodeURIComponent(place.id)}"><h2>${esc(place.name)}</h2></a><span>☆ Must visit</span><p>${esc(place.tag)}</p><small>🛵 ${esc(place.scooter.replace(/^Approx\.\s*/,''))}</small></div><button data-remove="${esc(place.id)}" aria-label="Remove ${esc(place.name)}">♥</button></article>`).join('');
