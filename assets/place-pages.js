(() => {
  ...
  const indexCard = place => { const travel = place.scooter.replace(/^Approx\.\s*/, '').replace(/by scooter/, 'away'); return `<article class="place-index-card"><a class="place-index-image" href="place.html?id=${encodeURIComponent(place.id)}"><img loading="lazy" decoding="async" src="${esc(place.image)}" alt="${esc(place.name)}"></a><div class="place-index-copy"><a href="place.html?id=${encodeURIComponent(place.id)}"><h2>${esc(place.name)}</h2></a><span class="place-index-badge">☆ Must visit</span><p>${esc(place.tag)}</p><div class="place-index-foot"><span>🛵 ${esc(travel)}</span>${saveButton(place)}</div></div></article>`; };
